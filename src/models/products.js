...
const selectAll = () => {
    return Pool.query(`SELECT products.id, products.name, products.stock, products.price, products.photo, products.descriptions, category.name as category_name, transactions.address as address_transactions FROM products INNER JOIN category ON products.category_id = category.id INNER JOIN transactions ON products.transactions_id = transactions.id;`);
}
const select =(id)=>{
    return Pool.query(`SELECT * FROM products where id='${id}'`);
}
const insert = (data) => {
    const { id,name,stock,price,photo,descriptions, category_id, transactions_id } = data
    return Pool.query(`INSERT INTO products(id,name,stock,price,photo,descriptions, category_id, transactions_id) VALUES('${id}','${name}',${stock},${price},'${photo}','${descriptions}', ${category_id}, ${transactions_id})`)
  }
const update = (data) => {
    const { id, name, stock, price, photo, descriptions, category_id, transactions_id } = data
    return Pool.query(`UPDATE products SET name='${name}', stock=${stock}, price=${price}, photo='${photo}', descriptions='${descriptions}', category_id=${category_id}, transactions_id=${transactions_id} WHERE id='${id}'`)
  }
